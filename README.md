...
> Response에는 동적으로 Application의 상태를 변경시킬 수 있도록 _Link를 통해 HATEOAS를 만족한다.
#### API 구조
...
1. 6개의 정상 Case 테스트.
2.잘못된 토큰을 가지고 자원에 접근할 경우 테스트
3. 게시글 저장할 때, 제목 조건을 만족하지 못할 경우 테스트.
4. 게시글 저장할 때, 내용 조건을 만족하지 못할 경우 테스트.
